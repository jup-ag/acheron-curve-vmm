...
pub fn calculate_fee_breakdown(
    ...
    if platform_fee_bps > MAX_BPS {
        return Err(anyhow!("Scale VMM: invalid platform fee"));
    }
    let mut total_bps = u32::from(platform_fee_bps);
    for beneficiary in beneficiaries {
        if beneficiary.share_bps > MAX_BPS {
            return Err(anyhow!("Scale VMM: invalid creator fee"));
        }
        total_bps = total_bps
    ...
    if total_bps > u32::from(MAX_BPS) {
        return Err(anyhow!("Scale VMM: invalid total fees"));
    }
    ...
    if breakdown.total_fee >= amount {
        return Err(anyhow!("Scale VMM: insufficient input after fees"));
    }
...
pub fn quote_buy(
    ...
    Ok(SwapComputation {
        ...
        amount_b: u64::try_from(output).map_err(|_| anyhow!("Scale VMM: math overflow"))?,
    })
}
...
pub fn quote_sell(
    ...
    Ok(SwapComputation {
        new_reserves_a: new_reserve_a_virtual,
        new_reserves_b: new_reserve_b,
        amount_a: u64::try_from(output).map_err(|_| anyhow!("Scale VMM: math overflow"))?,
        amount_b,
    })
}
...
fn calculate_fee(amount: u64, bps: u16) -> Result<u64> {
    let fee = u128::from(amount)
        .checked_mul(u128::from(bps))
        .ok_or_else(|| anyhow!("Scale VMM: fee overflow"))?
        .checked_div(u128::from(MAX_BPS))
        .ok_or_else(|| anyhow!("Scale VMM: fee overflow"))?;
    u64::try_from(fee).map_err(|_| anyhow!("Scale VMM: fee overflow"))
}
